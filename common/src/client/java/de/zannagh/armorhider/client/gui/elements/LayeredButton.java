...
public abstract class LayeredButton extends Button {
    ...
    @Nullable protected ResourceLocation midLayerSprite(boolean enabled) { return null; }

    protected ResourceLocation spriteBg() {
        //? if >= 1.21
        return this.isHoveredOrFocused() ? ResourceLocation.withDefaultNamespace("widget/button_highlighted") : ResourceLocation.withDefaultNamespace("widget/button");
        //? if < 1.21
        //return this.isHoveredOrFocused() ? new ResourceLocation("minecraft/textures/gui/widgets/button_highlighted.png") : new ResourceLocation("minecraft/textures/gui/widgets/button.png");
    }

    protected abstract void renderForeground(net.minecraft.client.gui.GuiGraphics guiGraphics, int mouseX, int mouseY, float a);

    //? if >= 1.21
    protected static ResourceLocation modSprite(String name) { return ResourceLocation.fromNamespaceAndPath("armor-hider", name); }
    //? if < 1.21
    //protected static ResourceLocation modSprite(String name) { return new ResourceLocation("armor-hider", name); }
    ...
    //? if >= 26.1-1.pre.1 {
    /*@Override
    protected void extractContents(net.minecraft.client.gui.GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, spriteBg(), this.getX(), this.getY(), this.width, this.height);
        if (midLayerSprite(isEnabled) instanceof ResourceLocation sprite) {
    ...
    *///?}
    ...
    /*@Override
    protected void renderContents(net.minecraft.client.gui.GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, spriteBg(), this.getX(), this.getY(), this.width, this.height);
        if (midLayerSprite(isEnabled) instanceof ResourceLocation sprite) {
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, this.getX() + (this.width - 15) / 2, this.getY() + (this.height - 15) / 2, 15, 15);
        }
    ...
    *///?}
    ...
    //? if <= 1.21.10 && >= 1.21.6 {
    /*@Override
    protected void renderWidget(net.minecraft.client.gui.GuiGraphics guiGraphics, int i, int j, float f) {
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, spriteBg(), this.getX(), this.getY(), this.width, this.height);
        if (midLayerSprite(isEnabled) instanceof ResourceLocation sprite) {
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, this.getX() + (this.width - 15) / 2, this.getY() + (this.height - 15) / 2, 15, 15);
        }
    ...
    *///?}
    ...
    public void renderWidget(net.minecraft.client.gui.GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (midSprite != null) {
            var texture = new ResourceLocation(midSprite.getNamespace(), "textures/gui/sprites/" + midSprite.getPath() + ".png");
