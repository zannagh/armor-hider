...
public final class PacketSender {
    ...
    public static void sendToPlayer(ServerPlayer player, SharedRuleNotificationPacket sharedRules) {
        Identifier channel = LegacyPacketHandler.getSharedRuleNotificationChannel();
        FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
        CompressedJsonCodec.encodeLegacy(sharedRules, buf);
        player.connection.send(new ClientboundCustomPayloadPacket(channel, buf));
    }
    *///?}
}
