...
public class ArmorRenderPipeline {
    ...
    private static ArmorModificationInfo tryResolveConfigFromPlayerEntityState(EquipmentSlot slot, PlayerEntity state){
        boolean isLocalPlayerEntityRenderState = state.getDisplayName() == null;
        return new ArmorModificationInfo(slot, ArmorHiderClient.CLIENT_CONFIG_MANAGER.getConfigForPlayer(isLocalPlayerEntityRenderState ? ArmorHiderClient.getCurrentPlayerName() : state.getDisplayName().getString()));
    }
